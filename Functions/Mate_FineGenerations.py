...
def Mate_FineGenerations(Population,smooth_peaks,Generations=5,NSelect=4):    
    for generation in np.arange(Generations):
        Population=Mate_square_GaussParPop(SeedPopulation=Population)
        Population=Mate_square_GaussParPop(SeedPopulation=Population)
        Population=RefineParametersPopulation(smooth_peaks=smooth_peaks,Population=Population)
