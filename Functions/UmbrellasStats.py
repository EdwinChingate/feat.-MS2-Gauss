...
def UmbrellasStats(smooth_peak,PeaksUmbrellaMat,NPeaks,RT_col=0,int_col=1,Points_for_regression=4):
    for pseudo_peak_id in np.arange(NPeaks,dtype='int'):
        ...
        Stats=RT_Gauss_std(PeakData=smooth_peak,MaxRTLoc=MaxRTLoc,EarlyLoc=EarlyLoc,LateLoc=LateLoc,RT_col=RT_col,int_col=int_col,Points_for_regression=Points_for_regression)
