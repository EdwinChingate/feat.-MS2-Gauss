...
def ResolvingChromatogram(Chromatogram,minSpec,minPoints=10,int_col=5,EarlyLoc=0,LateLoc=0,minWindow=11,minPoly=5):
    ...
    peaksMin=find_peaks(-SoftInt,prominence=1,distance=minPoints)[0]
    #plt.plot(PeakChr[:,2],PeakChr[:,1],'.')
    #plt.plot(PeakChr[toRegLoc,2],SoftInt,'-')
    #plt.plot(PeakChr[toRegLoc[peaksMin],2],PeakChr[toRegLoc[peaksMin],1],'o')
    #plt.show()    
    if len(peaksMin)<1:
